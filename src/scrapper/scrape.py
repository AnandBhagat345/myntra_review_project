...
class ScrapeReviews:
    def __init__(self,
                 ...
                 no_of_products: int):
        options = Options()
        # options.addd_argument("--no-sandbox")
        # options.add_arguement("--disable-dev-shm-usage")
        # options.add_arguement('--headleass')
    ...
    def scroll_to_load_reviews(self):
        ...
        self.driver.set_window_size(1920, 1080)    # Explain window size, adjust as

        #Get the initiatal height of the page
        last_height = self.driver.execute_script("return document.body.scrollHeight")


        # Scroll in smaller increments, waiting between scrolls
        while True:
            #scrolll down by a small amount
    ...
    def extract_products(self, product_reviews: list):
        try:
            ...
            Review_link = "http://www.myntra.com" + t2
            ...
            for i in range(len(user_rating)):
                try:
                    rating = (
                        user_rating[i]
                        .find("span", class_="user-review-startrating")
                        .get_text()
                        .strip()
                    )
                except:
    ...
    def skip_product(self, search_string, no_of_products, skip_index):
        product_urls: list = self.scrape_product_url(search_string, no_of_products)
    ...
    def get_review_data(self) -> pd.DataFrame:
        try:
            product_urls = self.scrape_product_url(self.product_name)

            if not product_urls:
                self.driver.quit()
                raise Exception("No products found on Myntra. Try another product name.")

            product_details = []
            index = 0

            while len(product_details) < self.no_of_products and index < len(product_urls):
            ...
            if not product_details:
                raise Exception("No reviews found for this product.")
